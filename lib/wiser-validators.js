...
function validateZoneDefinition(zone) {
  ...
  if (typeof zone.color !== 'undefined') {
    if (typeof zone.color !== 'string') {
      ...
      throw new Error(
        `${zone.name} color must be a hex value including a leading #`
      );
    }
  }
  ...
  if (!Array.isArray(zone.shape)) {
    throw new Error(`${zone.name} shape must be an array`);
  }
  if (zone.shape.length < 3) {
    throw new Error(`${zone.name} shape must have at least 3 points`);
  }
  for (let i = 0; i < zone.shape.length; i++) {
    ...
    if (
      ...
      throw new Error(
        `${
          zone.name
        } shape vertices must be contain x and y properties of type Number`
      );
    }
  }
}
