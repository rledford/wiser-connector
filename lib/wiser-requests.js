...
function getJSON(options) {
  return new Promise((resolve, reject) => {
    ...
    const req = (options.tlsEnabled ? https : http).request(options, res => {
...
function isServerAvailable(options) {
  return new Promise((resolve, reject) => {
    const { hostname, port, tlsEnabled } = options;

    const req = (tlsEnabled ? https : http).request(
      {
        hostname,
        port,
        path: '/wiser/api'
 ...
 * @param {*} options - hostname, port
 */
function getPassiveTagReport(options) {
  const { hostname, port, tlsEnabled } = options;
  return getJSON({
    hostname,
    port,
